...
public class Pergunta26 implements ActionListener{
		
	//Pergunta: Rearranje os elementos Ag, Ar e Ga arrastando-os para a tabela periódica.
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/26.jpg"));
	...
	*/
	...
	//Campo das respostas
	private ImageIcon imagemAgColorido = new ImageIcon(getClass().getResource("/imagens/imagensPerguntasElementosColoridos/47 Ag.jpg"));
	private ImageIcon imagemGaColorido = new ImageIcon(getClass().getResource("/imagens/imagensPerguntasElementosColoridos/31 Ga.jpg"));
	private ImageIcon imagemArColorido = new ImageIcon(getClass().getResource("/imagens/imagensPerguntasElementosColoridos/18 Ar.jpg"));
	...
	private MouseListener m1 = new MouseListener() {
		...
		public Pergunta26(InterfaceGrafica.Aluno aluno){
		...
		public void setDimensoes(){
			// Atribuindo dimensões, menos do Ag, Ar e Ga
			// 1A
		...
		public void setRespostas(){
			Ag.setBounds(686, 477, 39, 39);
			Ga.setBounds(446, 477, 39, 39);
			Ar.setBounds(565, 477, 39, 39);
			resposta1.setBounds(683, 179, 39, 39);
			resposta2.setBounds(761, 140, 39, 39);	//Estarão na tabela
			resposta3.setBounds(956, 101, 39, 39);
		...
		public void exibirRespostas(){
			resposta1.setIcon(imagemAgColorido);
			resposta2.setIcon(imagemGaColorido);
			resposta3.setIcon(imagemArColorido);
		}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(resposta1.getIcon()==imagemAg&&resposta2.getIcon()==imagemGa&&resposta3.getIcon()==imagemAr){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){	//Tempo equivalente a 1 Décimo de segundo
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(Ag.getIcon()!=imagemAg||Ga.getIcon()!=imagemGa||Ar.getIcon()!=imagemAr){
						Ag.setIcon(imagemAg);
						Ga.setIcon(imagemGa);
						Ar.setIcon(imagemAr);
					}
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta26.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta26.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta26.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta26.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta26.this.aluno.setResposta(new Boolean(false));
						if(Pergunta26.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta26.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaEnsinoMedio sort = new AuxSortearPerguntaEnsinoMedio();
							sort.proximaPergunta(Pergunta26.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta26.this.aluno.getIndiceDestrave()){
					try{
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta26.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta26.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta26.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta26.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
