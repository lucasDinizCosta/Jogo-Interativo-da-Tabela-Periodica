...
public class Pergunta23 implements ActionListener{
	
	//Pergunta: Coloque nos recipientes 3 elementos em estado gasoso à temperatura ambiente.
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/23.jpg"));
	...
	private JLabel recipiente1 = new JLabel("Elemento 1");
	private JLabel recipiente2 = new JLabel("Elemento 2");
	private JLabel recipiente3 = new JLabel("Elemento 3");
	private Font fonteEscritoRespostas = new Font("Arial Black",Font.BOLD,12);
	...
	public Pergunta23(InterfaceGrafica.Aluno aluno){
	...
	public void setRespostas(){
		recipiente1.setBounds(425, 529, 85, 15);
		recipiente2.setBounds(543, 529, 85, 15);
		recipiente3.setBounds(665, 529, 85, 15);
		recipiente1.setFont(fonteEscritoRespostas);
		recipiente2.setFont(fonteEscritoRespostas);
		recipiente3.setFont(fonteEscritoRespostas);
		recipiente1.setForeground(Color.RED);
		recipiente2.setForeground(Color.BLACK);
		recipiente3.setForeground(Color.BLUE);
		resposta1.setBounds(446, 477, 39, 39);
		resposta3.setBounds(565, 477, 39, 39);
		resposta2.setBounds(686, 477, 39, 39);
		...
		fundo.add(resposta3);
		fundo.add(resposta2);
		fundo.add(recipiente1);
		fundo.add(recipiente2);
		fundo.add(recipiente3);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			if((resposta1.getIcon()==resposta2.getIcon())||(resposta1.getIcon()==resposta3.getIcon())||(resposta2.getIcon()==resposta3.getIcon())){
				JOptionPane.showMessageDialog(null, "Coloque elementos diferentes nos recipientes.");
			}
			else{
				t.stop();
				paraThreads = true;
				...
				if(resposta1.getIcon()==imagemH||resposta1.getIcon()==imagemN||resposta1.getIcon()==imagemO||resposta1.getIcon()==imagemF||resposta1.getIcon()==imagemCl||resposta1.getIcon()==imagemHe||resposta1.getIcon()==imagemNe||resposta1.getIcon()==imagemAr||resposta1.getIcon()==imagemKr||resposta1.getIcon()==imagemXe||resposta1.getIcon()==imagemRn||resposta1.getIcon()==imagemUuo){
					if(resposta2.getIcon()==imagemH||resposta2.getIcon()==imagemN||resposta2.getIcon()==imagemO||resposta2.getIcon()==imagemF||resposta2.getIcon()==imagemCl||resposta2.getIcon()==imagemHe||resposta2.getIcon()==imagemNe||resposta2.getIcon()==imagemAr||resposta2.getIcon()==imagemKr||resposta2.getIcon()==imagemXe||resposta2.getIcon()==imagemRn||resposta2.getIcon()==imagemUuo){
						if(resposta3.getIcon()==imagemH||resposta3.getIcon()==imagemN||resposta3.getIcon()==imagemO||resposta3.getIcon()==imagemF||resposta3.getIcon()==imagemCl||resposta3.getIcon()==imagemHe||resposta3.getIcon()==imagemNe||resposta3.getIcon()==imagemAr||resposta3.getIcon()==imagemKr||resposta3.getIcon()==imagemXe||resposta3.getIcon()==imagemRn||resposta3.getIcon()==imagemUuo){
							if(pontosBonus>=0){
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta23.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta23.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta23.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta23.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta23.this.aluno.setResposta(new Boolean(false));
						if(Pergunta23.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta23.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaEnsinoMedio sort = new AuxSortearPerguntaEnsinoMedio();
							sort.proximaPergunta(Pergunta23.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta23.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta23.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta23.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta23.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta23.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
