...
public class Pergunta3 implements ActionListener{
	...
	//Pergunta: Coloque no pódio os elementos Pt, Ni e Ir em ordem crescente de acordo com a densidade.  
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/03.jpg"));
	...
	*/
	...
	public void exibirRespostas() {
		...
		terceiroLugar.setIcon(imagemNi);
		segundoLugar.setIcon(imagemPt);
		primeiroLugar.setIcon(imagemIr);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemNi&&segundoLugar.getIcon()==imagemPt&&primeiroLugar.getIcon()==imagemIr){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(Ni.getIcon()!=imagemNi||Pt.getIcon()!=imagemPt||Ir.getIcon()!=imagemIr){
						Ni.setIcon(imagemNi);
						Pt.setIcon(imagemPt);
						Ir.setIcon(imagemIr);
					}
