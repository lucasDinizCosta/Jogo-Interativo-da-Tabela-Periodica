...
public class Pergunta30 implements ActionListener{
	...
	//Pergunta: Coloque no pódio os elementos O, N e F em ordem decrescente de eletronegatividade.  
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/30.jpg"));
	...
	*/
	...
	public void exibirRespostas() {
		...
		segundoLugar.setIcon(imagemO);
		primeiroLugar.setIcon(imagemN);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemF&&segundoLugar.getIcon()==imagemO&&primeiroLugar.getIcon()==imagemN){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(N.getIcon()!=imagemN||O.getIcon()!=imagemO||F.getIcon()!=imagemF){
						N.setIcon(imagemN);
						O.setIcon(imagemO);
						F.setIcon(imagemF);
					}
