...
public class Pergunta38 implements ActionListener{
	
	//Pergunta: Coloque no tubo de ensaio um elemento químico artificial.
	...
	private JLabel resposta = new JLabel();
	private JLabel tuboDeEnsaio = new JLabel();
	private ImageIcon imagemFundo = new ImageIcon(getClass().getResource("/imagens/Fundo.jpg"));
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/38.jpg"));
	...
	*/
	...
	public Pergunta38(InterfaceGrafica.Aluno aluno){
	...
	public void setRespostas(){
		resposta.setBounds(629, 475, 39, 39);
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			if(resposta.getIcon()==imagemTc||resposta.getIcon()==imagemNp||resposta.getIcon()==imagemPu||resposta.getIcon()==imagemAm
			||resposta.getIcon()==imagemCm||resposta.getIcon()==imagemBk||resposta.getIcon()==imagemCf||resposta.getIcon()==imagemEs
			||resposta.getIcon()==imagemFm||resposta.getIcon()==imagemMd||resposta.getIcon()==imagemNo||resposta.getIcon()==imagemLr
			||resposta.getIcon()==imagemRf||resposta.getIcon()==imagemDb||resposta.getIcon()==imagemSg||resposta.getIcon()==imagemBh
			||resposta.getIcon()==imagemHs||resposta.getIcon()==imagemMt||resposta.getIcon()==imagemDs||resposta.getIcon()==imagemRg
			||resposta.getIcon()==imagemCn||resposta.getIcon()==imagemUut||resposta.getIcon()==imagemFl||resposta.getIcon()==imagemUup
			||resposta.getIcon()==imagemLv||resposta.getIcon()==imagemUus||resposta.getIcon()==imagemUuo){ 
			...
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção do tubo será válida, caso contrário a correção é finalizada para consumir menos processamento
					if(contadorParaLeitura > 0){	//Se o tempo de leitura for maior que zero significa que nenhuma resposta pode ser atribuida ainda
						resposta.setIcon(null);
						tuboDeEnsaio.setIcon(imagemTuboDeEnsaio);
					}
					else{
						if(tuboDeEnsaio.getIcon()!=imagemTuboDeEnsaio){
							resposta.setIcon(tuboDeEnsaio.getIcon());
							tuboDeEnsaio.setIcon(imagemTuboDeEnsaio);
						}
					}
				}
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta38.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta38.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta38.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta38.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta38.this.aluno.setResposta(new Boolean(false));
						if(Pergunta38.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta38.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaEnsinoMedio sort = new AuxSortearPerguntaEnsinoMedio();
							sort.proximaPergunta(Pergunta38.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta38.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta38.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta38.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta38.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta38.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
