...
public class Pergunta6 implements ActionListener{
	...
	//Pergunta: Coloque no pódio os elementos Cl, K e Na  em ordem crescente de acordo com o raio atômico.  
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/06.jpg"));
	...
	*/
	...
	public void exibirRespostas() {
		...
		terceiroLugar.setIcon(imagemCl);
		segundoLugar.setIcon(imagemNa);
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemCl&&segundoLugar.getIcon()==imagemNa&&primeiroLugar.getIcon()==imagemK){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(Cl.getIcon()!=imagemCl||K.getIcon()!=imagemK||Na.getIcon()!=imagemNa){
						Cl.setIcon(imagemCl);
						K.setIcon(imagemK);
						Na.setIcon(imagemNa);
					}
