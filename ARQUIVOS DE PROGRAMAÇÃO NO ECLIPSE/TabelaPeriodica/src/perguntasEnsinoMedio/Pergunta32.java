...
public class Pergunta32 implements ActionListener{
	...
	//Pergunta: Coloque no pódio os elementos O, Se e S em ordem crescente de raio atômico
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/32.jpg"));
	...
	public void exibirRespostas() {
		...
		terceiroLugar.setIcon(imagemO);
		segundoLugar.setIcon(imagemS);
		primeiroLugar.setIcon(imagemSe);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemO&&segundoLugar.getIcon()==imagemS&&primeiroLugar.getIcon()==imagemSe){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(O.getIcon()!=imagemO||Se.getIcon()!=imagemSe||S.getIcon()!=imagemS){
						O.setIcon(imagemO);
						Se.setIcon(imagemSe);
						S.setIcon(imagemS);
					}
