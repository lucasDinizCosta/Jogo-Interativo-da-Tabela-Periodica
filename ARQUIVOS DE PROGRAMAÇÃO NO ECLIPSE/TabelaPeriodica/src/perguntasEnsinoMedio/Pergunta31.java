...
public class Pergunta31 implements ActionListener{
	
	//Pergunta: Coloque no pódio os elementos Br, Fe, F, K e Ca em ordem crescente de raio atômico. 
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/31.jpg"));
	...
	*/
	...
	private ImageIcon imagemPodio = new ImageIcon(getClass().getResource("/imagens/Pódio5lugares.png"));
	private JLabel podio = new JLabel();
	private JLabel primeiroLugar = new JLabel();
	private JLabel segundoLugar = new JLabel();	
	private JLabel terceiroLugar = new JLabel();
	private JLabel quartoLugar = new JLabel();
	private JLabel quintoLugar = new JLabel();
	private ImageIcon imagemRecipiente = new ImageIcon(getClass().getResource("/imagens/Recipiente.png"));
	private JLabel fundoPrimeiroLugar = new JLabel();
	private JLabel fundoSegundoLugar = new JLabel();
	private JLabel fundoTerceiroLugar = new JLabel();
	private JLabel fundoQuartoLugar = new JLabel();
	private JLabel fundoQuintoLugar = new JLabel();
	
	/**
	 * Função do Mouse - Copia a imagem dos elementos para outros locais através
	 * do TransferHandler
	 */
	...
	public Pergunta31(InterfaceGrafica.Aluno aluno){
	...
	public void transferHandler() {
		...
		// CAMPO RESPOSTAS
		primeiroLugar.setTransferHandler(new TransferHandler("icon"));
		segundoLugar.setTransferHandler(new TransferHandler("icon"));
		terceiroLugar.setTransferHandler(new TransferHandler("icon"));
		quartoLugar.setTransferHandler(new TransferHandler("icon"));
		quintoLugar.setTransferHandler(new TransferHandler("icon"));
	}
	...
	public void setRespostas() {
		primeiroLugar.setBounds(673, 426, 39, 39);
		segundoLugar.setBounds(584, 450, 39, 39);
		terceiroLugar.setBounds(762, 465, 39, 39);
		quartoLugar.setBounds(495, 476, 39, 39);
		quintoLugar.setBounds(851, 483, 39, 39);
		fundoPrimeiroLugar.setBounds(662, 417, 61, 58);
		fundoSegundoLugar.setBounds(573, 441, 61, 58);
		fundoTerceiroLugar.setBounds(751, 456, 61, 58);
		fundoQuartoLugar.setBounds(484, 467, 61, 58);
		fundoQuintoLugar.setBounds(840, 474, 61, 58);
		fundoPrimeiroLugar.setIcon(imagemRecipiente);
		fundoSegundoLugar.setIcon(imagemRecipiente);
		fundoTerceiroLugar.setIcon(imagemRecipiente);
		fundoQuartoLugar.setIcon(imagemRecipiente);
		fundoQuintoLugar.setIcon(imagemRecipiente);
		podio.setBounds(470, 475, 444, 98);
		podio.setIcon(imagemPodio);
		fundo.add(fundoPrimeiroLugar);
		fundo.add(fundoSegundoLugar);
		fundo.add(fundoTerceiroLugar);
		fundo.add(fundoQuartoLugar);
		fundo.add(fundoQuintoLugar);
		fundo.add(primeiroLugar);
		fundo.add(segundoLugar);
		fundo.add(terceiroLugar);
		fundo.add(terceiroLugar);
		fundo.add(quartoLugar);
		fundo.add(quintoLugar);
		fundo.add(podio);
	}
	...
	public void exibirRespostas() {
		// Deixar os elementos que não são respostas opacos de modo que realce
		// apenas as respostas
		// Familia 1A
		H.setEnabled(false);
		Li.setEnabled(false);
		...
		//Colocando as imagens das respostas nos campos corretos
		quintoLugar.setIcon(imagemF);
		quartoLugar.setIcon(imagemBr);
		terceiroLugar.setIcon(imagemFe);
		segundoLugar.setIcon(imagemCa);
		primeiroLugar.setIcon(imagemK);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(quintoLugar.getIcon()==imagemF&&quartoLugar.getIcon()==imagemBr&&terceiroLugar.getIcon()==imagemFe&&segundoLugar.getIcon()==imagemCa&&primeiroLugar.getIcon()==imagemK){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(Br.getIcon()!=imagemBr||Ca.getIcon()!=imagemCa||Fe.getIcon()!=imagemFe||K.getIcon()!=imagemK||F.getIcon()!=imagemF){
						Br.setIcon(imagemBr);
						Ca.setIcon(imagemCa);
						Fe.setIcon(imagemFe);
						K.setIcon(imagemK);
					...
					if(contadorParaLeitura>0){	//Faz com que o jogador não responda antes do tempo de leitura das perguntas terminar
						primeiroLugar.setIcon(null);
						segundoLugar.setIcon(null);
						terceiroLugar.setIcon(null);
						quartoLugar.setIcon(null);
						quintoLugar.setIcon(null);
					}
				}
				else{
					parar();
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta31.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta31.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta31.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta31.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta31.this.aluno.setResposta(new Boolean(false));
						if(Pergunta31.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta31.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaEnsinoMedio sort = new AuxSortearPerguntaEnsinoMedio();
							sort.proximaPergunta(Pergunta31.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta31.this.aluno.getIndiceDestrave()){
					try{
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta31.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta31.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta31.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta31.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
