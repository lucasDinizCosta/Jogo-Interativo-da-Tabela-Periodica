...
public class Pergunta4 implements ActionListener{
	...
	//Pergunta: Coloque no pódio os elementos Cr, K e W em ordem crescente de acordo com o ponto de fusão.  
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/EnsinoMedio/04.jpg"));
	...
	*/
	...
	public void exibirRespostas() {
		...
		terceiroLugar.setIcon(imagemK);
		segundoLugar.setIcon(imagemCr);
		primeiroLugar.setIcon(imagemW);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemK&&segundoLugar.getIcon()==imagemCr&&primeiroLugar.getIcon()==imagemW){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(K.getIcon()!=imagemK||K.getIcon()!=imagemCr||W.getIcon()!=imagemW){
						K.setIcon(imagemK);
						Cr.setIcon(imagemCr);
						W.setIcon(imagemW);
					}
