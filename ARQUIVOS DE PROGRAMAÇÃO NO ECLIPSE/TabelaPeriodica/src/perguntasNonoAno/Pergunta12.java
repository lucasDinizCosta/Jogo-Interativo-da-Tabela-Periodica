...
public class Pergunta12 implements ActionListener{
	
	//Pergunta: Coloque no tubo de ensaio dois actinídeos da tabela periódica. 
	...
	//ELEMENTOS QUÍMICOS
		...
		private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/NonoAno/12.jpg"));
	...
	public Pergunta12(InterfaceGrafica.Aluno aluno){
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			if(resposta1.getIcon()==imagemAc||resposta1.getIcon()==imagemTh||resposta1.getIcon()==imagemPa||resposta1.getIcon()==imagemU||resposta1.getIcon()==imagemNp||resposta1.getIcon()==imagemPu||resposta1.getIcon()==imagemAm||resposta1.getIcon()==imagemCm||resposta1.getIcon()==imagemBk||resposta1.getIcon()==imagemCf||resposta1.getIcon()==imagemEs||resposta1.getIcon()==imagemFm||resposta1.getIcon()==imagemMd||resposta1.getIcon()==imagemNo|resposta1.getIcon()==imagemLr){
				...
				if(resposta2.getIcon()==imagemAc||resposta2.getIcon()==imagemTh||resposta2.getIcon()==imagemPa||resposta2.getIcon()==imagemU||resposta2.getIcon()==imagemNp||resposta2.getIcon()==imagemPu||resposta2.getIcon()==imagemAm||resposta2.getIcon()==imagemCm||resposta2.getIcon()==imagemBk||resposta2.getIcon()==imagemCf||resposta2.getIcon()==imagemEs||resposta2.getIcon()==imagemFm||resposta2.getIcon()==imagemMd||resposta2.getIcon()==imagemNo|resposta2.getIcon()==imagemLr){
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta12.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta12.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta12.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta12.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta12.this.aluno.setResposta(new Boolean(false));
						if(Pergunta12.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta12.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaNono sort = new AuxSortearPerguntaNono();
							sort.proximaPergunta(Pergunta12.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta12.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta12.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta12.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta12.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta12.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
