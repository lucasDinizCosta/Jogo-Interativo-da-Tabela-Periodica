...
public class Pergunta10 implements ActionListener{
	
	//Pergunta: Coloque no pódio os elementos Hg, Fe e O de acordo com o estado de agregação sólido, líquido e gasoso, respectivamente, à temperatura ambiente. 
	...
	//ELEMENTOS QUÍMICOS
		...
		private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/NonoAno/10.jpg"));
		...
		private JLabel liquido = new JLabel();
		private JLabel solido = new JLabel();	//Estes campos estarão nas posições dos elementos na tabela e tem como primeira imagem o recipiente 39x39
		private JLabel gasoso = new JLabel();
		...
		private JLabel escritoLiquido = new JLabel("Líquido");
		private JLabel escritoSolido = new JLabel("Sólido");
		private JLabel escritoGasoso = new JLabel("Gasoso");
		private Font fonteEstadosFisicos = new Font("Arial Black",Font.BOLD,12);
	...
	public Pergunta10(InterfaceGrafica.Aluno aluno){
	...
	public void transferHandler(){
		...
		//CAMPO RESPOSTAS
		liquido.setTransferHandler(new TransferHandler("icon"));
		solido.setTransferHandler(new TransferHandler("icon"));
		gasoso.setTransferHandler(new TransferHandler("icon"));
	}
	...
	public void setRespostas(){
		escritoLiquido.setBounds(438, 529, 70, 15);
		escritoSolido.setBounds(558, 529, 70, 15);
		escritoGasoso.setBounds(678, 529, 70, 15);
		escritoLiquido.setFont(fonteEstadosFisicos);
		escritoSolido.setFont(fonteEstadosFisicos);
		escritoGasoso.setFont(fonteEstadosFisicos);
		escritoLiquido.setForeground(Color.RED);
		escritoSolido.setForeground(Color.BLACK);
		escritoGasoso.setForeground(Color.BLUE);
		liquido.setBounds(446, 477, 39, 39);
		solido.setBounds(565, 477, 39, 39);
		gasoso.setBounds(686, 477, 39, 39);
		...
		fundo.add(solido);
		fundo.add(gasoso);
		fundo.add(escritoLiquido);
		fundo.add(escritoSolido);
		fundo.add(escritoGasoso);
	}
	...
	public void exibirRespostas(){
		liquido.setIcon(imagemHg);
		solido.setIcon(imagemFe);
		gasoso.setIcon(imagemO);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(liquido.getIcon()==imagemHg&&solido.getIcon()==imagemFe&&gasoso.getIcon()==imagemO){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta10.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta10.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta10.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta10.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta10.this.aluno.setResposta(new Boolean(false));
						if(Pergunta10.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta10.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaNono sort = new AuxSortearPerguntaNono();
							sort.proximaPergunta(Pergunta10.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta10.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta10.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta10.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta10.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta10.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
