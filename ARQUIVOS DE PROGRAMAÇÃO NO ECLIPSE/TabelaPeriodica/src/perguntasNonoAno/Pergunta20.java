...
public class Pergunta20 implements ActionListener{
	
	//Pergunta: Coloque no tubo de ensaio dois elementos que pertencem ao quinto período. 
	...
	//ELEMENTOS QUÍMICOS
		...
		private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/NonoAno/20.jpg"));
	...
	public Pergunta20(InterfaceGrafica.Aluno aluno){
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			if(resposta1.getIcon()==imagemRb||resposta1.getIcon()==imagemSr||resposta1.getIcon()==imagemY||resposta1.getIcon()==imagemZr||resposta1.getIcon()==imagemNb||resposta1.getIcon()==imagemMo||resposta1.getIcon()==imagemTc||resposta1.getIcon()==imagemRu||resposta1.getIcon()==imagemRh||resposta1.getIcon()==imagemPd||resposta1.getIcon()==imagemAg||resposta1.getIcon()==imagemCd||resposta1.getIcon()==imagemIn||resposta1.getIcon()==imagemSn||resposta1.getIcon()==imagemSb||resposta1.getIcon()==imagemTe||resposta1.getIcon()==imagemI||resposta1.getIcon()==imagemXe){ 
				...
				if(resposta2.getIcon()==imagemRb||resposta2.getIcon()==imagemSr||resposta2.getIcon()==imagemY||resposta2.getIcon()==imagemZr||resposta2.getIcon()==imagemNb||resposta2.getIcon()==imagemMo||resposta2.getIcon()==imagemTc||resposta2.getIcon()==imagemRu||resposta2.getIcon()==imagemRh||resposta2.getIcon()==imagemPd||resposta2.getIcon()==imagemAg||resposta2.getIcon()==imagemCd||resposta2.getIcon()==imagemIn||resposta2.getIcon()==imagemSn||resposta2.getIcon()==imagemSb||resposta2.getIcon()==imagemTe||resposta2.getIcon()==imagemI||resposta2.getIcon()==imagemXe){
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta20.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta20.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta20.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta20.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta20.this.aluno.setResposta(new Boolean(false));
						if(Pergunta20.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta20.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaNono sort = new AuxSortearPerguntaNono();
							sort.proximaPergunta(Pergunta20.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta20.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta20.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta20.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta20.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta20.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
