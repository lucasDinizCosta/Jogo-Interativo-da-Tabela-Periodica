...
public class Pergunta4 implements ActionListener{
	
	//Pergunta: Coloque no pódio os elementos S, Br e Cl em ordem crescente de massa atômica.
	...
	private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/NonoAno/04.jpg"));
	...
	*/
	...
	private ImageIcon imagemPodio = new ImageIcon(getClass().getResource("/imagens/Pódio3lugares.png"));
	...
	public Pergunta4(InterfaceGrafica.Aluno aluno){
	...
	public void setRespostas() {
		primeiroLugar.setBounds(628, 427, 39, 39);
		segundoLugar.setBounds(539, 451, 39, 39);
		terceiroLugar.setBounds(717, 466, 39, 39);
		fundoPrimeiroLugar.setBounds(617, 417, 61, 58);
		fundoSegundoLugar.setBounds(528, 441, 61, 58);
		fundoTerceiroLugar.setBounds(706, 456, 61, 58);
		//primeiroLugar.setBorder(BorderFactory.createLineBorder(Color.BLACK));
		//segundoLugar.setBorder(BorderFactory.createLineBorder(Color.BLACK));
		//terceiroLugar.setBorder(BorderFactory.createLineBorder(Color.BLACK));
		...
		podio.setBounds(514, 475, 266, 98);
	...
	public void exibirRespostas() {
		...
		terceiroLugar.setIcon(imagemS);
		segundoLugar.setIcon(imagemCl);
		primeiroLugar.setIcon(imagemBr);
	}
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==responder){
			...
			if(terceiroLugar.getIcon()==imagemS&&segundoLugar.getIcon()==imagemCl&&primeiroLugar.getIcon()==imagemBr){	//se as imagens dos espacos forem iguais as imagens dos elementos faça o ponto virar 200, OU SEJA ACERTO
				if(pontosBonus>=0){
					exibirRespostas();
		...
		Timer t2 = new Timer(10, new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Enquanto o usuário não tiver respondido ou a próxima pergunta não for liberada a correção da tabela será válida, caso contrário a reparação é finalizada para consumir menos processamento
					if(S.getIcon()!=imagemS||Br.getIcon()!=imagemBr||Cl.getIcon()!=imagemCl){
						S.setIcon(imagemS);
						Br.setIcon(imagemBr);
						Cl.setIcon(imagemCl);
					}
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta4.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta4.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta4.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta4.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta4.this.aluno.setResposta(new Boolean(false));
						if(Pergunta4.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta4.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaNono sort = new AuxSortearPerguntaNono();
							sort.proximaPergunta(Pergunta4.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta4.this.aluno.getIndiceDestrave()){
					try{
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta4.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta4.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta4.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta4.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
