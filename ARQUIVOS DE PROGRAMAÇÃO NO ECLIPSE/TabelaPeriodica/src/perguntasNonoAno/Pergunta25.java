...
public class Pergunta25 implements ActionListener{
	
	//Pergunta: Coloque no tubo de ensaio o principal elemento presente nos ossos. 
	...
	//ELEMENTOS QUÍMICOS
		...
		private ImageIcon imagemPergunta = new ImageIcon(getClass().getResource("/imagens/imagensEnunciados/NonoAno/25.jpg"));
	...
	public Pergunta25(InterfaceGrafica.Aluno aluno){
		...
		Timer tempoDestraveAntesHora = new Timer(500, new ActionListener(){	//Tempo de análise de 0,5 segundos
			public void actionPerformed(ActionEvent e){
				if(paraThreads==false){	//Caso o botão responder não foi clicado a variável continuará falsa
					if(indice2 <= Pergunta25.this.aluno.getIndiceDestrave()){ 	//Se o indice2 for menor ou igual ao do aluno, significa que está aguardando um novo destrave
						try{	//Tratar excessões existentes
							if(caminho.toFile().exists()){	//Se o arquivo de texto existe com Destravador existe através da rede
								...
								if(indice2 == Pergunta25.this.aluno.getIndiceDestrave()){	//Se os destraves do arquivo estão conforme o do aluno, qualquer outro significa iniciar nova pergunta
					...
					else if(indice2 > Pergunta25.this.aluno.getIndiceDestrave()){	//Se o destrave da próxima pergunta foi feito e o botão responder não foi clicado
						t.stop();
						tempoDestraveAntesHora.stop();
						Pergunta25.this.aluno.setRegistroPontosPerguntas(0);
						Pergunta25.this.aluno.setResposta(new Boolean(false));
						if(Pergunta25.this.aluno.getSizeRegistroPontosPerguntas()<10){
						...
						else if(Pergunta25.this.aluno.getSizeRegistroPontosPerguntas()>=10){
							janela.dispose();
							AuxSortearPerguntaNono sort = new AuxSortearPerguntaNono();
							sort.proximaPergunta(Pergunta25.this.aluno);
						}
					}
				}
	...
	public class verificadorDeDestraveProximaPergunta extends Thread implements ActionListener{	//VERIFICA SE O DESTRAVE FOI FEITO E O ARQUIVO EXISTE
		...
		Timer t5 = new Timer(1000, new ActionListener(){	//Representa 1 segundo
			public void actionPerformed(ActionEvent e){
				if(indice3 <= Pergunta25.this.aluno.getIndiceDestrave()){
					try{	//esse erro tratado seria caso a URL seja inválida
						if(caminho.toFile().exists()){	//Se o arquivo de texto existe
							...
							if((indice3==10)&&(Pergunta25.this.aluno.getSizePerguntaIndividual()==0)){	//Representa o destrave da ultima pergunta
								t5.stop();
							}
							if(indice3==Pergunta25.this.aluno.getIndiceDestrave()){	//Se o número de destraves do arquivo for equivalente ao do aluno será necessário um novo destrave
				...
				else if(indice3 > Pergunta25.this.aluno.getIndiceDestrave()){
		...
		public void JanelaTransicao(){
			//JANELA
			if(Pergunta25.this.aluno.getSizePerguntaIndividual()>0){	//Enquanto houver perguntas a serem respondidas
